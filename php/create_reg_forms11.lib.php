...
$db=mysql_connect("localhost","sahil","sahil");
...
else
{
  ...
  $java="<script language=\"javascript\">
		...
		{ 
			var f=document.frm;
			";
  for($x=1;($x<=mysql_num_fields($result)-1)&&($a_row1[$x]!=NULL);$x++)
    {
  ...
  //predefined fields 
  $querystring="EXPLAIN pragyan_reg_".$form_id;
  //echo $querystring;
  $result=mysql_query($querystring);	
  if(!$result)
    {	
    ...
    {
      while($a=mysql_fetch_assoc($result))/*to get the names of the predefined fields*/
	{
	  $fieldname=$a['Field'];
	  if(substr($fieldname,0,4)=="pre_")
	    {
	      ...
	      if($fieldname=="pre_name")
		{
		  $java.="\n if(f.pre_name.value==\"\")
				...
				";	      
		  $temp.="\n\t<TR><TD width=40%>Please enter your name :</TD>";	
	  ...
	  if($fieldname=="pre_dob")
	    {
	      $java.="\n f.pre_dob.value=f.year.value+'-'+f.month.value+'-'+f.day.value;
				...
				";	      
	      $temp.="\n\t<TR><TD width=40%>Day : </td><td><input type=hidden name=pre_dob><select name=day>";
		   for($i=1;$i<=31;$i++)	
		  {
