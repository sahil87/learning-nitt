...
tree *p, *q;			//DISPLAY p and q GLOBALLY IDIOT!!!!! //p is the active pointer
void inorder (tree *);
void postorder (tree *);
void preorder (tree *);
void
copyTree (tree * current, tree * newCurrent);
void inorder_stacks (tree *);
void preorder_stacks (tree *);
void heightOfTree (tree *);
void copyTree (tree * current, tree * newCurrent);
int height = 0, sub = 0;
int selectNode ();
int displayNode ();
...
copyTree (tree * current, tree * newCurrent)
{
	newCurrent->info = current->info;
	if (current->childLeft != NULL)
	{
		q = new tree;
		q->childLeft = NULL;
		q->childRight = NULL;
		newCurrent->childLeft = q;
		q->parent = newCurrent;
		copyTree (current->childLeft, q);
	}
	if (current->childRight != NULL)
	{
		...
		q->childLeft = NULL;
		q->childRight = NULL;
		newCurrent->childRight = q;
		q->parent = newCurrent;
		copyTree (current->childRight, q);
	}
}

void
heightOfTree (tree * current)
{
	//if(current==NULL) return 0;
	//else if(current->childLeft==NULL && current->childRight==NULL)	return 1;
	//else return (1 + MAX(heightOfTree(childLeft),heightTree(childRight)));
...
preorder_stacks (tree * current)
{
	...
	while (1)
	{
		...
		{
			if (s->size () == 0 && current->childRight == 0)//second condition not required?
				break;
			current = s->pop ();
		}
	}
...
void
inorder_stacks (tree * current)
{
	stack *s = new stack;
	enum d
	{ UPL, DOWN } direction = DOWN;
	while (1)
	{
...
displayNode ()
{
	...
	if (NULL == p->childLeft)
		...
		cout << "\t" << p->childLeft->info;
	if (NULL == p->childRight)
		cout << "\t\tNULL\n";
	else
		cout << "\t\t" << p->childRight->info << "\n";
	return 0;
}
...
inorder (tree * current)
{
	...
	if (current->childLeft != NULL)
		inorder (current->childLeft);
	cout << current->info << " ";
	if (current->childRight != NULL)
		inorder (current->childRight);
}
...
postorder (tree * current)
{
	...
	if (current->childLeft != NULL)
		postorder (current->childLeft);
	if (current->childRight != NULL)
		postorder (current->childRight);
	cout << current->info << " ";
}
